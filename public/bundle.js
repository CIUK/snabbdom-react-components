(function () {
	...
	function init(modules, api) {
	  ...
	  function createElm(vnode$$1, insertedVnodeQueue) {
	    ...
	    let elm; let children = vnode$$1.children; let
	      sel = vnode$$1.sel;
	    if (isDef(sel)) {
	      // Parse selector
	  ...
	  function patchVnode(oldVnode, vnode$$1, insertedVnodeQueue) {
	    ...
	    let elm = vnode$$1.elm = oldVnode.elm; let oldCh = oldVnode.children; let
	      ch = vnode$$1.children;
	...
	function applyRemoveStyle(vnode, rm) {
	  ...
	  var name; var elm = vnode.elm; var i = 0;
	...
	var eventlisteners = {create: updateEventListeners, update: updateEventListeners};
	...
	const componentFn = (viewObject) => {
	    ...
	    if (profile === 'function') {
	      return function () {
	        ...
	        forEach$1(hook, (fn) => {
	          ...
	          if (name === 'reducer') {
	            result[name] = isFunction$2(fn) ? fn(state, action, cmpnt, ...rest) : fn;
	          } else {
	            result[name] = isFunction$2(fn) ? fn(state, cmpnt, action, ...rest) : fn;
	          }
	        });
	  ...
	  const registerHooks = () => {
	    forEach$1(viewObject.hooks, (hook, name) => {
	      if (name === 'items') {
	...
	const JsonToVNode = (json, context) => {
	  ...
	  if (isArray_1(json.children)) {
	    children = map_1(json.children, c => JsonToVNode(c, context));
	...
	const TodoList = props => createComponent_1({
	  render: () => (
	    h_1('form', {
	      on: {
	...
	const TodoItems = props => createComponent_1({
	  createTasks(item) {
	    return h_1('li', {
	      key: item.key,
	      on: {
	  ...
	  render(state, component) {
	    ...
	    return h_1('ul', listItems);
	  },
	});

	const Todo = (params = {}) => createComponent_1({
	  key: 'todo',
	  state: () => ({
	    items: params.items || [],
	  ...
	  render: (state, component) => h_1('div', [
	    TodoList({
	...
	const FormBox = styled_1$1.form``;
	const Input = styled_1$1.input``;
	const Button = styled_1$1.button``;

	const TodoList$1 = props => createComponent_1({
	  render: () => (
	    FormBox({
	      on: {
	        submit: props.addItem,
	...
	const ListBox = styled_1$1.ul``;
	const ListItem = styled_1$1.li``;

	const TodoItems$1 = props => createComponent_1({
	  createTasks(item) {
	    return ListItem({
	      key: item.key,
	      on: {
	...
	const TodoContainer = styled_1$1.div``;

	const Todo$1 = (params = {}) => createComponent_1({
	  key: 'todoStyled',
	  state: () => ({
	    items: params.items || [],
	    currentItem: { text: '', key: '' },
	...
	const AppBox = styled_1$1.div``;
	const AppMenu = styled_1$1.ul``;
	const AppMenuItem = styled_1$1.li`
  cursor: pointer;
`;
	const EmptyApp = styled_1$1.span``;
	const Break = styled_1$1.hr``;
	...
	const App = createComponent_1({
	  ...
	  getAppsList: (state, component) => map_1(examples, (example, name) => AppMenuItem({
	    on: {
